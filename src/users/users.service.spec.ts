...
describe('UsersService', () => {
  ...
  describe('updateUserByEmail', () => {
    it('should update user and driver info by email', async () => {
      ...
      jest.spyOn(prisma.driver, 'update').mockResolvedValue({
        ...
        expirationDateBankAccount: new Date(`${updateData.expirationDateBankAccount}-01T00:00:00.000Z`),
        phone: updateData.phone,
        licenseNumber: updateData.licenseNumber,
      });

      const result = await service.updateUserByEmail('test@example.com', updateData);

      expect(result.updatedUser.address).toBe(updateData.address);
      expect(result.updatedDriver.bankAccountNumber).toBe(updateData.bankAccountNumber);
    });
  ...
  describe('create', () => {
    ...
    it('should throw error if RFC already exists', async () => {
      ...
      await expect(service.create(createUserDto)).rejects.toThrow(
        `El correo electrónico new@example.com ya está registrado`
      );
    });
  });
